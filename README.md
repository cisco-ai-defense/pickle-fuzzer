...
A structure-aware test case generator for Python pickle parsers and validators. `pickle-whip` generates complex, valid pickle bytecode across all protocol versions (0-5) for use in fuzzing and testing pickle parsing implementations.

## Project Description

`pickle-whip` is a Rust-based tool designed to help security researchers and developers test Python pickle parsing implementations. Unlike traditional fuzzers that generate random bytes, `pickle-whip` understands pickle's structure and generates syntactically valid pickle bytecode that exercises edge cases, complex opcode sequences, and protocol-specific features.
...
The binary will be available at `target/release/pickle-whip`.
...
pickle-whip --dir samples --samples 100
...
```
Usage: pickle-whip [OPTIONS] [FILE]
...
`pickle-whip` provides Python bindings for integration with Python-based fuzzing tools like Atheris.
...
```python
from pickle_whip import Generator
...
## Fuzzing pickle-whip Itself

`pickle-whip` includes comprehensive fuzz targets for testing its own generation logic using cargo-fuzz (libFuzzer).
...
`pickle-whip` can be used to fuzz any Python project that parses pickle data.
...
import atheris
import sys
from pickle_whip.fuzzer import fuzz_pickle_parser
...
`pickle-whip` uses a stack-based approach to generate valid pickle bytecode:
...
`pickle-whip` uses a simulation-based approach to generate valid pickle bytecode:
...
`pickle-whip` is highly optimized for fast pickle generation with excellent scalability.
...
pickle-whip --seed 42 output.pkl

# Use faster protocols (V1 or V2)
pickle-whip --protocol 1 output.pkl

# Smaller opcode ranges generate faster
pickle-whip --min-opcodes 10 --max-opcodes 50 output.pkl
```
...
**Important**: `pickle-whip` generates potentially malicious pickle data for testing purposes only. 
...
For contributors and developers working on `pickle-whip`:
...
- [fuzz/README.md](fuzz/README.md) - Fuzzing pickle-whip itself with cargo-fuzz
...
### Getting Help
- **Issues**: Report bugs or request features via [GitHub Issues](https://github.com/cisco-ai-defense/pickle-whip/issues)
- **Discussions**: Ask questions and share ideas in [GitHub Discussions](https://github.com/cisco-ai-defense/pickle-whip/discussions)
...
Project Link: [https://github.com/cisco-ai-defense/pickle-whip](https://github.com/cisco-ai-defense/pickle-whip)
